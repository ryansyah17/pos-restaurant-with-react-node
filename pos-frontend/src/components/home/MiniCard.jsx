...
const MiniCard = ({title, icon, number, footerNum}) => {
  return (
    <div className='bg-[#1a1a1a] py-5 px-5 rounded-lg w-[50%]'>
      ...
      <div>
        <h1 className='text-[#f5f5f5] text-4xl font-bold mt-5'>{title === 'Total Earnings' ? `R${number}` : number}</h1><h1 className='text-[#ababab] text-sm'></h1>
        <h1 className='text-[#f5f5f5] text-lg mt-2'><span className='text-[#02ca3a]'>{footerNum}%</span></h1>
      </div>
    </div>
  )
}
